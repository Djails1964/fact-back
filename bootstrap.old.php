...
if (!defined('APP_INITIALIZED')) {
    ...
    */
            ...
            if ($isCrossDomainRequest && $isAllowedOrigin) {
                ...
                if (!empty($origin)) {
                    ...
                    if (count($hostParts) >= 2) {
                        ...
                        if (stripos($originHost, 'DEV-facturation') !== false) {
                            // Retrait du point initial pour les domaines locaux sans extension
                            $cookieDomain = 'DEV-facturation'; 
                        } elseif (stripos($originHost, 'localhost') !== false) {
    ...
    // 1. Récupérer les origines autorisées depuis le .env
    $corsOriginEnv = env('CORS_ORIGIN', '');
    $allowedOrigins = array_map('trim', explode(',', $corsOriginEnv));
    ...
    // En développement, si l'origine n'est pas dans la liste, on peut l'autoriser pour débloquer
    elseif ($isDev && !empty($origin)) {
        header("Access-Control-Allow-Origin: $origin");
    }

    // Headers obligatoires pour Axios/React
